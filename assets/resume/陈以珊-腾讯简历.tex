...
\begin{center}
  ...
  \small AI Application Engineer $|$ LLM \& Multi-Agent Systems $|$ Workflow Automation \\ \vspace{1pt}
  \small \href{mailto:Kristyn921207@gmail.com}{Kristyn921207@gmail.com} $|$ 微信: kristyn5942 $|$ \href{https://kristynchen.github.io/Personal-Website/}{\underline{个人网站}}
\end{center}
\vspace{-12pt}

% 个人简介
\section{个人简介}
\small {电子与计算机工程专业学生，专注于 LLM 应用开发与多智能体系统 (Agentic AI) 设计。具备 Prompt Engineering、RAG 与 AI 工作流自动化的实战经验。主导构建多智能体协同系统与自动化测试框架，将大语言模型能力落地至实际场景，实现复杂任务的自动化处理与效率优化。}
\vspace{-4pt}
...
\resumeSubHeadingListStart
  \resumeProjectHeading
    ...
    \resumeItemListStart
      \resumeItem{参与 LangGraph 向 MCP 架构迁移，主导构建基于 LLM 的自动化测试与评估框架：设计 Prompt Engineering 与意图识别机制，通过多维度评估标准将系统意图识别准确率从 27\% 提升至 86\%，实现测试反馈分钟级闭环。}
      \resumeItem{设计并实现多智能体测试与调度系统，构建 Benchmark 与自动化评估 pipeline，实现测试数据与可视化看板的实时联动。}
      \resumeItem{基于视觉大模型（VLM）构建结构化信息提取与异常检测 pipeline，将非结构化文档转化为可用数据流，减少约 \textbf{90\%} 人工审核成本，推动 AI 能力在业务场景中的工程化落地。}
    \resumeItemListEnd
...
\resumeSubHeadingListStart
  ...
  \resumeProjectHeading
    {\textbf{\href{https://ternswipewebapp-production.up.railway.app/}{terny.ai} — AI 科研机会匹配系统} \quad | \quad Python, LangGraph, LLM}{2025年11月 -- 至今}
    \resumeItemListStart
      \resumeItem{构建基于爬虫与大语言模型的科研信息采集系统，实现对复杂实验室网站的非结构化网页数据自动化解析与结构化抽取。}
      \resumeItem{设计并实现 \textbf{RAG (检索增强生成)} 架构：结合向量检索与 “Excel 权威数据优先 + WebFallback” 的混合检索策略，将识别准确度从 75\% 提升至 98\% 以上。}
      \resumeItem{主导多智能体 (Multi-Agent) 系统设计：利用 LangGraph 构建协同流水线，实现研究方向理解、能力建模与双向匹配决策的自动化处理。}
    \resumeItemListEnd
  ...
  \resumeProjectHeading
    ...
    \resumeItemListStart
      \resumeItem{完成数据清洗、特征工程与多模型集成，实现约 \textbf{0.36 RMSE}，排名\textbf{前 2 名}；使用 Excel 制作统计表辅助结果分析，熟练运用数据透视表与公式进行数据整理与报表呈现。}
    \resumeItemListEnd
...
%\textbf{AI Frameworks}：LangGraph, OpenAI API, VLM, Hugging Face \\
%\textbf{Data \& Retrieval}：Embedding, Vector Search (向量数据库), Hybrid Retrieval (混合检索) \\
%\textbf{Automation}：n8n, Workflow Orchestration, API Integration, Python, SQL \\
%\textbf{语言能力}：中文（母语），英语（流利），日语（会话）

\textbf{AI \& LLM}：Prompt Engineering, RAG, Multi-Agent Systems, LLM Application Development \\
\textbf{Frameworks \& Tools}：LangGraph, MCP, VLM, Hugging Face \\
\textbf{Backend \& Data}：Python, SQL, API Integration, Workflow Orchestration, Embedding, Vector Search, Hybrid Retrieval \\
\textbf{Others}：GNNs, Transformers, pandas, NumPy, Excel \quad | \quad 中文（母语），英语（流利），日语（会话）
